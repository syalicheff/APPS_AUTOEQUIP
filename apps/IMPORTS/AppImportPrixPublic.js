...
module.exports.sqlModifOffi = async function (tabData,file,dernierFichier){
  try
  {
    ...
    await console.log("Doublons supprimés ")
    var tabDataInsert = []
    ...
    var CreationArticleFournisseurTampon = "BULK INSERT [AUTO_EQUIP].[dbo].[F_IMPORT] FROM 'C:/DATA/DOSSIER_FTP/testSQL/Import SAGE.txt' WITH (FIRSTROW = 2,FIELDTERMINATOR = '0x09' ,ROWTERMINATOR='0x0a')"
    ...
    if(tabData[4].DESIGNATIONMODIF == "NON")
    {
      ...
      "Art.[AR_PrixVen] = Imp.[AR_PrixVen],\n"+// ON MET A JOUR DANS TOUS LES CAS
      "Art.[AR_PrixAch] = Imp.[AR_PrixAch],\n"+ // ON MET A JOUR DANS TOUS LES CAS
    ...
    if(tabData[4].DESIGNATIONMODIF == "OUI")
    {
      var MajsArticles =
      "MERGE [F_ARTICLE] AS Art\n"+
      "USING [F_IMPORT] AS Imp\n"+
